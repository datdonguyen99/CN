...
class Client:
    ...
    def createWidgets(self):
        ...
        def on_enter_play(e):
            self.start['background'] = '#02a6a6'
        def on_leave_play(e):
            self.start['background'] = '#6c8a2c'
        def on_enter_pause(e):
            self.pause['background'] = '#ffcbf2'
        def on_leave_pause(e):
            self.pause['background'] = '#b31d92'
        def on_enter_stop(e):
            self.stop['background'] = '#d48f11'
        def on_leave_stop(e):
            self.stop['background'] = '#eb0559'
        ...
        def on_enter_describe(e):
            self.describe['background'] = '#17996e'
        def on_leave_describe(e):
            self.describe['background'] = '#a0f2d7'
        def on_enter_switch(e):
            self.switch['background'] = '#999c19'
        def on_leave_switch(e):
            self.switch['background'] = '#e9ed02'
        def on_enter_teardown(e):
            self.teardown['background'] = '#940f1e'
        def on_leave_teardown(e):
            self.teardown['background'] = '#f05164'
        ...
        self.teardown.grid(row=3, column=0, padx=2, pady=2)
        self.teardown.bind("<Enter>", on_enter_teardown)
        self.teardown.bind("<Leave>", on_leave_teardown)

        # TODO: Create a stop button
        # Create stop button
        self.stop = Button(self.master, width=20, padx=3, pady=3,bg='#ffffff', activebackground='#deaaff')
        self.stop["text"] = "Stop"
        self.stop["command"] = self.stopMovie
        self.stop.grid(row=2, column=2, padx=2, pady=2)
        self.stop.bind("<Enter>", on_enter_stop)
        self.stop.bind("<Leave>", on_leave_stop)
        ...
        self.describe.grid(row=3, column=1, padx=2, pady=2)
        ...
        self.dropbar.grid(row=4, column=1, padx=2, pady=2)
        ...
        self.switch.grid(row=4, column=0, padx=2, pady=2)
        self.switch.bind("<Enter>", on_enter_switch)
        self.switch.bind("<Leave>", on_leave_switch)

        # Create a label to display the movie
        self.label = Label(self.master, height=19, bg='#e7e8c1')
        self.label.grid(row=0, column=0, columnspan=4,
                        sticky=W + E + N + S, padx=5, pady=5)

        # TODO Create a place to display remaining time
        self.timer = Entry(self.master, width=20, justify='center', bg='#e7e8c1',
                           textvariable=self.remainingTime)
        self.timer.grid(row=1, column=2, padx=2, pady=2)
    ...
    def updateCountDownTimer(self):
        ...
        self.progressBar['value'] = (self.totalTime - remainingTime) * 100 / self.totalTime
    ...
    def parseRtspReply(self, data):
        ...
        if self.requestSent == self.SETUP or self.requestSent == self.SWITCH:
            ...
            self.total.grid(row=3, column=2, padx=2, pady=2)
            self.total["bg"] = '#e7e8c1'
